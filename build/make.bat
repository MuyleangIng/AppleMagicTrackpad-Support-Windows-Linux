...
echo   NuGet.exe restore ..\MagicTrackpadHidFilter\packages.config -PackagesDirectory ..\packages\
echo Also please install Rust from https://rustup.rs if you want the Rust desktop UI packaged.
...
set /p "sha1TpUi=Please enter the SHA1 Thumbprint for signing the Rust UI exe: "
set /p "sha1TpDr=Please enter the SHA1 Thumbprint for signing the drivers: "

msbuild ..\MagicTrackpadUsbDriver\MagicTrackpad2PtpDevice.vcxproj /p:Configuration=Release /p:Platform=ARM64
if %errorlevel% neq 0 exit /b %errorlevel%

msbuild ..\MagicTrackpadUsbDriver\MagicTrackpad2PtpDevice.vcxproj /p:Configuration=Release /p:Platform=x64
if %errorlevel% neq 0 exit /b %errorlevel%

msbuild ..\MagicTrackpadHidFilter\AmtPtpHidFilter.vcxproj /p:Configuration=Release /p:Platform=ARM64
if %errorlevel% neq 0 exit /b %errorlevel%

msbuild ..\MagicTrackpadHidFilter\AmtPtpHidFilter.vcxproj /p:Configuration=Release /p:Platform=x64
if %errorlevel% neq 0 exit /b %errorlevel%

cargo build --release -p mt2-win-ui
if %errorlevel% neq 0 exit /b %errorlevel%
...
copy ..\target\release\mt2-win-ui.exe result\MagicTrackpadRs.exe
copy ..\MagicTrackpadUsbDriver\build\AmtPtpDeviceUsbUm\x64\Release\AmtPtpDeviceUsbUm.dll result\AMD64
copy ..\MagicTrackpadUsbDriver\build\AmtPtpDeviceUsbUm\ARM64\Release\AmtPtpDeviceUsbUm.dll result\ARM64
copy ..\MagicTrackpadHidFilter\build\AmtPtpHidFilter\x64\Release\AmtPtpHidFilter.sys result\AMD64
copy ..\MagicTrackpadHidFilter\build\AmtPtpHidFilter\ARM64\Release\AmtPtpHidFilter.sys result\ARM64
mkdir result\pdb\AMD64
mkdir result\pdb\ARM64
copy ..\MagicTrackpadUsbDriver\build\AmtPtpDeviceUsbUm\x64\Release\AmtPtpDeviceUsbUm.pdb result\pdb\AMD64
copy ..\MagicTrackpadUsbDriver\build\AmtPtpDeviceUsbUm\ARM64\Release\AmtPtpDeviceUsbUm.pdb result\pdb\ARM64
copy ..\MagicTrackpadHidFilter\build\AmtPtpHidFilter\x64\Release\AmtPtpHidFilter.pdb result\pdb\AMD64
copy ..\MagicTrackpadHidFilter\build\AmtPtpHidFilter\ARM64\Release\AmtPtpHidFilter.pdb result\pdb\ARM64

signtool sign /v /fd sha256 /sha1 "%sha1TpUi%" /t http://timestamp.digicert.com result\MagicTrackpadRs.exe
if %errorlevel% neq 0 exit /b %errorlevel%
